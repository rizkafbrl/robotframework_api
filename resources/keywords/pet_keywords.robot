...
Get Pets By Status
    [Arguments]        ${petStatus}
    ${response}=       GET On Session    petstore    ${FIND_PETS_BY_STATUS_ENDPOINT}    params=status=${petStatus}
    RETURN             ${response}

Get Pet By ID
    [Arguments]        ${petID}
    ${response}=       GET On Session    petstore    ${RETRIEVE_PET_BY_ID_ENDPOINT}/${petID}
    RETURN             ${response}

Verify Pets Status
    [Arguments]        ${response}       ${expectedPetStatus}
    ${pets}=           Get Json Value    ${response.json()}    $[*]
    FOR     ${pet}    IN    @{pets}
        Should Be Equal As Strings       ${pet}[status]        ${expectedPetStatus}
    END

Verify Pet By ID
    [Arguments]        ${response}       ${expectedPetID}
    ${pets}=           Get Json Value    ${response.json()}    $[*]
    Run Keyword If    len(${pets}) == 0    Fail    No pets found in the response.
    ${pet}=           Set Variable       ${pets}[0]
    Log               Verifying pet: ${pet}     console=True
    Should Be Equal As Numbers    ${pet}[id]    ${expectedPetID}
